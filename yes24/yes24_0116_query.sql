...
-- 특정 저자의 모든 책을 조회하세요.
-- SELECT title FROM Books WHERE author = '자청 저'

-- 특정 출판사가 출판한 책들을 조회하세요.
-- SELECT title FROM Books WHERE publisher = '웅진';
-- 
-- 조인관계
...
-- SELECT author, AVG(rating) AS avg_rating FROM Books GROUP BY author ORDER BY avg_rating ;
-- ​
-- 국내도서랭킹이 1위인 책의 제목과 저자를 조회하세요.
-- SELECT * FROM Books WHERE ranking_weeks = 1;
-- ​
...
-- FROM Books
-- GROUP BY author
-- ORDER BY num_books DESC   -- 구분해서 카운트
-- LIMIT 1;
...
-- SELECT MONTH(publishing) as month, COUNT(*) as num_books, AVG(price) as avg_price  # 년도 데이터만 가져옴
-- FROM Books
-- GROUP BY month;
